from django.shortcuts import render, redirect#render:套用模版的类   redirect:把请求重定向使用的类
from django.contrib import auth  # Django用户认证（Auth）组件：一般用在用户的登录注册上，用于判断当前的用户是否合法
from django.contrib.auth.decorators import login_required#在函数上使用@login_required,访问某个页面的时候必须处于登录状态
from .forms import UserForm#引入我们创建的表格类，一个是用户名，一个是密码
import traceback
...
from .models import CaseSuite
from .models import SuiteCase
from .models import TestCaseExecuteResult
from .task import case_task
from . import models
import json
from .models import CaseSuiteExecuteRecord
...
@login_required
def project(request):
    print("request.user.is_authenticated: ", request.user.is_authenticated)  # 打印用户是否已登录
    projects = Project.objects.filter().order_by('-id')  # 使用负id是为了倒序取出项目数据
    print("projects:", projects)  # 打印项目名称
    return render(request, 'project.html', {'projects': get_paginator(request, projects)})

@login_required
def index(request):#登录后的首页，需要在templates下有个index.html的模版文件
    ...
    if request.method == "POST":
        ...
        if case_suite_list:
            ...
            for suite_id in case_suite_list:
                ...
                suite_task.apply_async((test_suite_record.id, test_suite.id, server_address), countdown=count_down_time)
        else:
            print("运行测试集合用例失败")
...
def login(request):
    ...
    if request.session.get('is_login', None):#登录成功后，is_login会有个值，这个if就会触发访问首页的重定向
        return redirect('/')
    # 如果是表单提交行为，则进行登录校验
    if request.method == "POST":#请求是个Post请求
        login_form = UserForm(request.POST)#把请求的字段写入到form
        message = "请检查填写的内容！"
        if login_form.is_valid():#判断表单中的内容是否合法
            username = login_form.cleaned_data['username']#从username字段中安全取出username值
            password = login_form.cleaned_data['password']#从password字段中安全取出password值
            try:
                # 使用django提供的身份验证功能
