    ...
    for test_case_id in test_case_id_list:
        ...
        try:
            if request_method.lower()=="get":
                res_data = request_process(url, request_method, request_data)
            else:
...
# 用例集合执行
@shared_task
def suite_task(case_suite_record_id, case_suite_id, server_address):
    case_suite_record =  CaseSuiteExecuteRecord.objects.get(id=case_suite_record_id)
    case_suite = CaseSuite.objects.get(id=case_suite_id)
